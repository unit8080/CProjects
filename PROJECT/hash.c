...
void HashDelete (HashTable *table, int ipaddr)
{
    HashNode *node; 
    HashNode *prev = NULL; 
    int hashIdx = hashIndex(table, ipaddr);
    ...
    while (node != NULL) {
        if (node->ipaddr == ipaddr) {
            if (prev != NULL) {
                ...
                if (table->array[hashIdx].head == node)
                    table->array[hashIdx].head = node->next;
            }
            free(node);
            return;
        }
...
{
    table = createHashTable(HASH_BUCKET_SIZE, HASH_MODULO);
#if 0
